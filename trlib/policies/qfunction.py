...
class FittedQ(QFunction):
    ...
    def persisted_values(self, s_prime_a, absorbing):
        vals = self.values(s_prime_a)
        vals[absorbing == 1] = 0
        return vals
...
class DiscreteFittedQ(QFunction):
    ...
    def action_values(self, states, action):
        for a in self._actions:
            if a == action:
                return self._regressors[a].predict(states)
    ...
    def _state_action_values(self, states, actions):
        assert np.shape(states)[0] == np.shape(actions)[0]
        n_samples = np.shape(states)[0]
        vals = np.empty(n_samples)
        for a in self._actions:
            mask = actions == a
            mask = np.transpose(mask)[0]
            vals[mask] = self._regressors[a].predict(states[mask])
        return vals
