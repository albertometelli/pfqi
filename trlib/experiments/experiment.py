...
class RepeatExperiment(Experiment):
    ...
    def _run_algorithm(self, n_run, seed = None):
        ...
        return self._algorithm.run(self._n_steps, n_run, callbacks, pre_callbacks, **self._algorithm_params)
    ...
    def run(self, n_jobs = 1):
        ...
        if n_jobs == 1:
            results = [self._run_algorithm(i) for i in range(self._n_runs)]
        elif n_jobs > 1:
